import { createOneSpotThunk } from "../../../store/spots";
...
export default function CreateSpot() {
  ...
  const onSubmit = async (e) => {
    ...
    if (description.length < 30) {
      error.description =
        "Description is required and needs to be at least 30 characters long.";
    }
    if (!name) {
    ...
    if (previewImage) {
      if (
        ...
        error.previewImage =
          "The image url needs to end in .jpeg, .png, or jpeg";
    }
  ...
  return (
    <div className="page-wrapper-create-spot">
      <form className="create-spot-form" onSubmit={onSubmit}>
        ...
        {error?.previewImage && (
          <span className="error">{error?.previewImage}</span>
        )}
        <input
          type="text"
          value={previewImage}
        ...
        <label className="create-spot-label">
          Image URL (Upload in jpeg, jpg, or png format only.)
        </label>
